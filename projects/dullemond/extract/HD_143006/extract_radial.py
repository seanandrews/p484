...
print('IMPORTANT NOTE: SPECIAL PROCEDURE FOR HD 143006')
...
if s=='y' or s=='Y':
    ...
    q.ix0      = 1507  # Note that x is here the 0th index, y the 1st index (opposite to imshow)
    q.iy0      = 1503
...
#
# SPECIAL PURPOSE PART HERE: CUT OUT THE BLOB
#
icutrange = [190,250]
a.circim[icutrange[0]:icutrange[1],:]=floorvalue
nphirest  = a.nphi-(icutrange[1]-icutrange[0])
...
params   = [np.linspace(0,rmax,300),np.linspace(0.,90.,300),np.linspace(-90,90.,300)]
#parstart = [0.25,20.8,22.3]  # The inner ring
parstart = [0.416,24.1,12.9]  # The outer ring (but this one has a blob, which is cut out)
...
axes[1].set_ylim((1e-3*a.circim.max(),a.circim.max()))
...
#
r0           = params_start[0]
params_lims  = np.array([(0.8*r0,1.1*r0),(0.,40.),(-90,90.)])
...
a.circim_average = a.circim_rescaled.sum(axis=0)/nphirest
...
for ir in range(a.nr):
    a.circim_rms[ir] = np.sqrt(((a.circim_rescaled[:,ir]-a.circim_average[ir])**2).sum()/nphirest)
