...
name     = 'HD_163296'
dpc      = 101.         # Distance to HD 163296 (2018.05.25 from Wiki)

print('IMPORTANT NOTE: HD 163296 IMAGE CENTER DONE BY HAND')
...
if s=='y' or s=='Y':
    ...
    q.ix0      = 1002  # Note that x is here the 0th index, y the 1st index (opposite to imshow)
    q.iy0      = 1002
...
axes[1].set_ylim((1e-3*a.circim.max(),3e-1*a.circim.max()))
...
#
r0           = params_start[0]
params_lims  = np.array([(0.9*r0,1.1*r0),(40.,52.),(0.,180.)])

#
# The half-width of the random population in each of the parameters
#
params_dran  = np.array([1e-2*params_start[0],3.,20.])
