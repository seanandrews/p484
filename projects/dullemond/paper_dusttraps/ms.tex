...
\hline
% AS 209     & 121              & 0.912        &  0.631 \\ % Data from 2018.05.13 Table
AS 209     & 121              & 0.832        &  1.413 \\ % 2018.07.02
Elias 24   & 136              & 0.776        &  6.03  \\ % 2018.07.02
% HD 163296  & 101              & 2.14         &  25.7  \\ % Data from 2018.05.13 Table
HD 163296  & 101              & 2.04         &  17.0  \\ % 2018.07.02
GW Lup     & 155              & 0.457        &  0.331 \\ % 2018.07.02
% HD 143006  & 166              & 0.813        &  0.871 \\ % Data estimated by Kees May/June 2018
...
HD 143006  & 1 &  35 &  45 &   3.21 &  41.05 &  72.20 &   5.06 &   4.42 &  27.2 &  22.0 &   2.1 &   0.16 &  10.5 &   0.57\\
HD 143006  & 2 &  59 &  72 &   2.48 &  64.83 & 112.51 &   7.88 &   7.49 &  21.6 &  16.6 &   2.0 &   0.16 &  26.5 &   1.44\\
\hline
\hline
\end{tabular}
\end{center}
\caption{\label{tab-gauss-params}The model parameters for the Gaussian ring fits
  in Figs.~\ref{fig-obs-profiles} and \ref{fig-obs-gaussfits}. The values
  $r_{\mathrm{in}}$ and $r_{\mathrm{out}}$ are the inner and outer boundary of
  the fitting domain. $A$ is the peak linear brightness temperature of the
  best-fit Gaussian ring model, $r_0$ is its radius in $\mathrm{au}$,
  ...
  $\mathrm{au}$. The last six columns are derived from a simple disk model
  assumption: $T_{\mathrm{d}}$ is the midplane temperature of the disk (we
  assume gas and dust temperature to be equal) computed from
  ...
  temerature $T_{\mathrm{d,br,lin}}=B_\nu(T_{\mathrm{d}})c^2/2\nu^2$ where
  $\nu=c/0.13\,\mathrm{cm}$ is the frequency belonging to the wavelength of 0.13
  $\mathrm{cm}$ of ALMA band 6.  The fourth-to-last column expresses the
  deconvolved dust ring width in units of the disk pressure scale height $h_p$
  computed from $T_{\mathrm{d}}$. The $\tau$ column gives the estimated optical
  depth at the peak of the ring, calculated from
  $\tau=-\ln(1-A/T_{\mathrm{d,br,lin}})$.  The last two columns are the computed
  dust mass in the ring in units of Earth masses, assuming the small grain
  absorption opacity
  ...
  of \citet{1997MNRAS.291..121I} ($M_{\mathrm{d,iv}}$). In making these
  estimates, the optically thin approximation was used. See Appendix
  \ref{sec-computing-dust-mass} for details. An estimate of the
  optical-depth-corrected dust mass can be made by multiplying the mass values
  in the table by $\tau/(1-e^{-\tau})$.}
\end{table*}
