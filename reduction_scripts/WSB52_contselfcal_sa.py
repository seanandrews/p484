...
SB1: 2016.1.00484.L/WSB_52_a_06_TM1  
     Observed 14 May 2017, 17 May 2017, and 19 May 2017 (3 execution blocks)
     PI: S. Andrews
     As delivered to PI
LB1: 2016.1.00484.L/WSB_52_a_06_TM1 
     Observed 9 September 2017 and 6 October 2017 (2 execution blocks)
     PI: S. Andrews
     As delivered to PI
...
prefix = 'WSB52' #string that identifies the source and is at the start of the name for all output files
...
data_params = {'SB1': {'vis' : '/data/sandrews/LP/2016.1.00484.L/science_goal.uid___A001_Xbd4641_X1e/group.uid___A001_Xbd4641_X25/member.uid___A001_Xbd4641_X26/calibrated/calibrated_final.ms',
               ...
               'LB1': {'vis' : '/data/sandrews/LP/2016.1.00484.L/science_goal.uid___A001_X8c5_X64/group.uid___A001_X8c5_X65/member.uid___A001_X8c5_X66/calibrated/calibrated_final.ms', 
                       'name' : 'LB1',
                       'field' : 'WSB_52',
...
for i in data_params.keys():      
    ...
    # Averaged continuum dataset saved to WSB52_SB1_initcont.ms
    # Flagchannels input string for LB1: '3:1858~1984, 7:1858~1984'
    # Averaged continuum dataset saved to WSB52_LB1_initcont.ms
...
mask_semiminor = 0.4 #semiminor axis of mask in arcsec
mask_ra = '16h27m39.42s'
mask_dec = '-24.39.15.97'
...
if not skip_plots:
    ...
    #Peak of Gaussian component identified with imfit: ICRS 16h27m39.421955s -24d39m15.96902s

    fit_gaussian(prefix+'_SB1_initcont_exec1.image', region = SB1_mask)
    #Peak of Gaussian component identified with imfit: ICRS 16h27m39.421645s -24d39m15.95193s

    fit_gaussian(prefix+'_SB1_initcont_exec2.image', region = SB1_mask)
    #Peak of Gaussian component identified with imfit: ICRS 16h27m39.420371s -24d39m15.98297s

    fit_gaussian(prefix+'_LB1_initcont_exec0.image', region = LB1_mask)
    #Peak of Gaussian component identified with imfit: ICRS 16h27m39.421487s -24d39m15.97625s

    fit_gaussian(prefix+'_LB1_initcont_exec1.image', region = LB1_mask)
    #Peak of Gaussian component identified with imfit: ICRS 16h27m39.421259s -24d39m15.97490s
    #PA of Gaussian component: 138.45 deg
    #Inclination of Gaussian component: 54.73 deg
...
(Also check the corresponding plots.)
	...
	LB1, EB0: J1733-1304 = 1.588 Jy at 232.582 GHz
	LB1, EB1: J1733-1304 = 1.895 Jy at 232.585 GHz
...
au.getALMAFlux('J1733-1304', frequency = '232.582GHz', date = '2017/09/10')     # LB1, EB0
au.getALMAFlux('J1733-1304', frequency = '232.588GHz', date = '2017/10/06')     # LB1, EB1
...
LB1, EB0:
Closest Band 3 measurement: 3.110 +- 0.050 (age=-6 days) 91.5 GHz
Closest Band 7 measurement: 1.200 +- 0.040 (age=+5 days) 343.5 GHz
getALMAFluxCSV(): Fitting for spectral index with 1 measurement pair of age 17 days from 2017/09/10, with age separation of 0 days
  2017/08/24: freqs=[91.46, 343.48], fluxes=[2.8, 1.08]
Median Monte-Carlo result for 232.582000 = 1.426080 +- 0.153117 (scaled MAD = 0.150884)
Result using spectral index of -0.719951 for 232.582 GHz from 3.110 Jy at 91.460 GHz = 1.588298 +- 0.153117 Jy

LB1, EB1:
Closest Band 3 measurement: 3.070 +- 0.080 (age=+4 days) 103.5 GHz
Closest Band 3 measurement: 3.250 +- 0.090 (age=+4 days) 91.5 GHz
Closest Band 7 measurement: 1.490 +- 0.080 (age=+4 days) 343.5 GHz
getALMAFluxCSV(): Fitting for spectral index with 1 measurement pair of age 4 days from 2017/10/06, with age separation of 0 days
  2017/10/02: freqs=[103.49, 91.46, 343.48], fluxes=[3.07, 3.25, 1.49]
Median Monte-Carlo result for 232.588000 = 1.885678 +- 0.162059 (scaled MAD = 0.161992)
Result using spectral index of -0.593201 for 232.588 GHz from 3.160 Jy at 97.475 GHz = 1.886420 +- 0.162059 Jy
...
PA = 138.0 #these are the rough values pulled from Gaussian fitting and used for initial deprojection. They are NOT the final values used for subsequent data analysis
incl = 55.0
phasecenter = au.radec2deg('16:27:39.430000, -24.39.15.54000')
peakpos = au.radec2deg('16:27:39.421, -24.39.15.975')
offsets = au.angularSeparation(peakpos[0], peakpos[1], phasecenter[0], phasecenter[1], True)
"""
(0.00012554777850294944,
 -3.7500000059219594e-05,
 -0.00012083333333508573,
 -3.408152347386306e-05)
"""
offx = 3600.*offsets[3]
offy = 3600.*offsets[2]
...
if not skip_plots:
    ...
    #The ratio of the fluxes of WSB52_SB1_initcont_exec0.vis.npz to WSB52_SB1_initcont_exec1.vis.npz is 0.95496
    #The scaling factor for gencal is 0.977 for your comparison measurement
    #The error on the weighted mean ratio is 8.046e-04, although it's likely that the weights in the measurement sets are too off by some constant factor
    ...
    #The ratio of the fluxes of WSB52_SB1_initcont_exec2.vis.npz to WSB52_SB1_initcont_exec1.vis.npz is 1.00134
    #The scaling factor for gencal is 1.001 for your comparison measurement
    #The error on the weighted mean ratio is 7.657e-04, although it's likely that the weights in the measurement sets are too off by some constant factor
    ...
    #The ratio of the fluxes of WSB52_LB1_initcont_exec1.vis.npz to WSB52_SB1_initcont_exec1.vis.npz is 1.08421
    #The scaling factor for gencal is 1.041 for your comparison measurement
    #The error on the weighted mean ratio is 1.112e-03, although it's likely that the weights in the measurement sets are too off by some constant factor

    #We replot the deprojected visibilities with rescaled factors to check that the values make sense
    plot_deprojected([prefix+'_SB1_initcont_exec0.vis.npz', prefix+'_SB1_initcont_exec1.vis.npz', prefix+'_SB1_initcont_exec2.vis.npz', prefix+'_LB1_initcont_exec0.vis.npz', prefix+'_LB1_initcont_exec1.vis.npz'],
                     fluxscale=[1/0.925, 1, 1, 1, 1/1.08], offx = offx, offy = offy, PA = PA, incl = incl, show_err=False)

#now correct the flux scales
rescale_flux(prefix+'_SB1_initcont_exec0.ms', [0.961])
rescale_flux(prefix+'_LB1_initcont_exec1.ms', [1.041])
#Splitting out rescaled values into new ms: WSB52_XB1_initcont_execX_rescaled.ms
...
#WSB52_SB_contp0.image
#Beam 0.268 arcsec x 0.225 arcsec (88.41 deg)
#Flux inside disk mask: 66.88 mJy
#Peak intensity of source: 38.38 mJy/beam
#rms: 9.60e-02 mJy/beam
#Peak SNR: 399.66
...
#WSB52_SB_contp1.image
#Beam 0.268 arcsec x 0.225 arcsec (88.43 deg)
#Flux inside disk mask: 67.64 mJy
#Peak intensity of source: 41.65 mJy/beam
...
#WSB52_SB_contp2.image
#Beam 0.268 arcsec x 0.225 arcsec (88.46 deg)
#Flux inside disk mask: 67.82 mJy
#Peak intensity of source: 42.01 mJy/beam
...
#WSB52_SB_contap.image
#Beam 0.268 arcsec x 0.224 arcsec (88.03 deg)
#Flux inside disk mask: 67.83 mJy
...
#WSB52_combined_contp0.image
#Beam 0.044 arcsec x 0.034 arcsec (84.45 deg)
#Flux inside disk mask: 68.12 mJy
...
LB1_obs0_timerange = '2017/09/09/00~2017/09/12/00'
LB2_obs1_timerange = '2017/10/05/00~2017/10/08/00'
...
#WSB52_combined_contp1.image
#Beam 0.044 arcsec x 0.034 arcsec (84.45 deg)
#Flux inside disk mask: 67.62 mJy
#Peak intensity of source: 3.53 mJy/beam
...
#WSB52_combined_contp2.image
#Beam 0.044 arcsec x 0.034 arcsec (84.45 deg)
#Flux inside disk mask: 67.63 mJy
...
#WSB52_combined_contp3.image
#Beam 0.044 arcsec x 0.034 arcsec (84.45 deg)
#Flux inside disk mask: 67.68 mJy
...
#WSB52_combined_contp4.image
#Beam 0.044 arcsec x 0.034 arcsec (84.45 deg)
#Flux inside disk mask: 67.72 mJy
#Peak intensity of source: 3.76 mJy/beam
#rms: 1.21e-02 mJy/beam
#Peak SNR: 310.71

# continued improvement (in both map quality and SNR, and not just peak SNR)
...
tclean_wrapper(vis = combined_cont_p5+'.ms' , imagename = combined_cont_p5, mask = common_mask, scales = LB_scales, threshold = '0.025mJy', savemodel = 'modelcolumn')
estimate_SNR(combined_cont_p5+'.image', disk_mask = common_mask, noise_mask = noise_annulus)
#WSB52_combined_contp5.image
#Beam 0.044 arcsec x 0.034 arcsec (84.45 deg)
#Flux inside disk mask: 67.30 mJy
#Peak intensity of source: 3.86 mJy/beam
#rms: 1.20e-02 mJy/beam
#Peak SNR: 322.87

# still improved, but now a marginal change.  ready for amp self-cal.
...
#WSB52_combined_contap.image
#Beam 0.043 arcsec x 0.034 arcsec (87.64 deg)
#Flux inside disk mask: 67.08 mJy
